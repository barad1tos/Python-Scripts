...
Usage:
    ...
    6. Manage memory with analytics.clear_old_events()
    7. Merge data from multiple instances with analytics.merge_with()

Example:
    ```python
    from utils.analytics import Analytics

    # Initialize with configuration and loggers
...
class Analytics:
    ...
    def __init__(self, config: Dict[str, Any], console_logger, error_logger, analytics_logger, max_events: int = None):
        """
    ...
    def clear_old_events(self, days: int = 7) -> int:
        ...
        self.events = [e for e in self.events if datetime.strptime(e["Start Time"], self.time_format) >= cutoff_date]
