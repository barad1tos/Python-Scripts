...
class EnhancedRateLimiter:
    ...
    def __init__(
        ...
        self.semaphore: Optional[asyncio.Semaphore] = None  # Initialized as None
    ...
    async def initialize(self) -> None:
        ...
        if self.semaphore is None:
            try:
                ...
                self.logger.error(f"Error initializing RateLimiter semaphore: {e}", exc_info=True)
    ...
    async def acquire(self) -> float:
        ...
        """
        if self.semaphore is None:
            self.logger.error("RateLimiter semaphore not initialized. Call initialize() first.")
...
class AppleScriptClient:
    ...
    def __init__(self, config: dict, console_logger: logging.Logger = None, error_logger: logging.Logger = None):
        ...
        self.semaphore: Optional[asyncio.Semaphore] = None  # Initialize as None
    ...
    async def initialize(self) -> None:
        ...
        if self.apple_scripts_dir is None:
            self.error_logger.critical("AppleScript directory is not set. Cannot initialize client.")
        ...
        if self.semaphore is None:
            try:
                ...
                if concurrent_limit <= 0:
                    self.error_logger.critical(f"Invalid 'apple_script_concurrency' value in config: {concurrent_limit}. Must be positive.")
                    raise ValueError(f"Invalid 'apple_script_concurrency' value: {concurrent_limit}")
                self.semaphore = asyncio.Semaphore(concurrent_limit)
                self.console_logger.debug(f"AppleScriptClient semaphore initialized with concurrency: {concurrent_limit}")
            except Exception as e:
                self.error_logger.error(f"Error initializing AppleScriptClient semaphore: {e}", exc_info=True)
                raise  # Re-raise to signal initialization failure

        self.console_logger.info("AppleScriptClient asynchronous initialization complete.")
    ...
    async def run_script(self, script_name: str, arguments: Union[List[str], None] = None, timeout: Optional[float] = None) -> Optional[str]:
        ...
        """
        if self.apple_scripts_dir is None:
            self.error_logger.error("AppleScript directory is not set. Cannot run script.")
            return None
        ...
        args_str = f"args: {', '.join(f'{arg}' for arg in arguments)}" if arguments else "no args"
        ...
        async with self.semaphore:
            try:
                ...
                try:
                    ...
                    if stderr:
                        stderr_text = stderr.decode('utf-8', errors='ignore').strip()  # Decode safely
                        # Log stderr as warning, might contain useful info even on success
                        self.console_logger.warning("◁ %s stderr: %s", script_name, stderr_text[:200])  # Log a bit more for debug

                    if proc.returncode != 0:
                        # Log the full stderr on failure
                        self.error_logger.error(
                            "◁ %s failed with return code %s: %s", script_name, proc.returncode, stderr.decode('utf-8', errors='ignore').strip()
                        )
                        return None

                    result = stdout.decode('utf-8', errors='ignore').strip()  # Decode safely
                    ...
                    try:
                        proc.kill()
                        await asyncio.wait_for(proc.wait(), timeout=5)  # Wait a bit for process to terminate
                        self.console_logger.debug("Process for %s killed after timeout.", script_name)
                    except (ProcessLookupError, asyncio.TimeoutError):
                        self.console_logger.warning("Could not kill or wait for process %s after timeout.", script_name)
                    ...
                    try:
                        ...
                        self.console_logger.debug("Process for %s killed after cancellation.", script_name)
                    except (ProcessLookupError, asyncio.TimeoutError):
                        self.console_logger.warning("Could not kill or wait for process %s after cancellation.", script_name)

                    raise  # Re-raise cancellation
                except (subprocess.CalledProcessError, OSError) as e:
                    # These exceptions might occur during communicate() or process creation
                    self.error_logger.error("⊗ %s error during execution: %s", script_name, e)
                    ...
                    return None
                except Exception as e:  # Catch any other unexpected errors during communication/waiting
                    self.error_logger.exception(f"⊗ {script_name} unexpected error during communicate/wait: {e}")
                ...
                return None
            except Exception as e:  # Catch any other unexpected errors before process creation
                self.error_logger.exception(f"⊗ {script_name} unexpected error before process creation: {e}")
                return None
    ...
    def _format_script_preview(self, script_code):
        ...
        tell_match = re.search(r'tell application\s+["\'](.*?)["\']', script_code, re.IGNORECASE)  # Use IGNORECASE
        if tell_match:
            app_name = tell_match.group(1)
            # Include first part of the command for better context
            command_preview = script_code[tell_match.end() :].strip()[:30] + "..."  # noqa E501
            return f'tell application "{app_name}" {command_preview}'

        # Fallback if pattern not found
        return script_code[:50] + "..." if len(script_code) > 50 else script_code  # Show a bit more fallback context
    ...
    async def run_script_code(self, script_code: str, arguments: Union[List[str], None] = None, timeout: Optional[float] = None) -> Optional[str]:
        ...
        :return: The output of the script, or None if an error occurred
        """
        if not isinstance(script_code, str) or not script_code.strip():  # Basic validation
        ...
        self.console_logger.info("▷ inline-script '%s' (%dB) [t:%ss]", code_preview, len(script_code.encode('utf-8')), timeout)  # Use byte length
        ...
        async with self.semaphore:
            try:
                ...
                try:
                    ...
                    if stderr:
                        stderr_text = stderr.decode('utf-8', errors='ignore').strip()  # Decode safely
                        self.console_logger.warning("◁ inline-script stderr: %s", stderr_text[:200])  # Log a bit more

                    if proc.returncode != 0:
                        self.error_logger.error(
                            "◁ inline-script failed with return code %s: %s", proc.returncode, stderr.decode('utf-8', errors='ignore').strip()
                        )
                        return None
                    ...
                    self.console_logger.info(
                        "◁ inline-script (%dB, %.1fs) %s", len(result.encode('utf-8')), elapsed, result_preview
                    )  # Use byte length
                    return result
                    ...
                    try:
                        proc.kill()
                        await asyncio.wait_for(proc.wait(), timeout=5)  # Wait a bit
                        self.console_logger.debug("Process for inline-script killed after timeout.")
                    except (ProcessLookupError, asyncio.TimeoutError):
                        self.console_logger.warning("Could not kill or wait for process for inline-script after timeout.")
                    return None
                except asyncio.CancelledError:
                    ...
                    try:
                        proc.kill()
                        await asyncio.wait_for(proc.wait(), timeout=5)  # Wait a bit
                        self.console_logger.debug("Process for inline-script killed after cancellation.")
                    except (ProcessLookupError, asyncio.TimeoutError):
                        self.console_logger.warning("Could not kill or wait for process for inline-script after cancellation.")

                    raise  # Re-raise cancellation
                except (subprocess.SubprocessError, OSError) as e:
                    self.error_logger.error("⊗ inline-script error during execution: %s", e)
                    ...
                    self.error_logger.exception(f"⊗ inline-script unexpected error during communicate/wait: {e}")
                    # Attempt to kill the process
                    try:
                        proc.kill()
                        await asyncio.wait_for(proc.wait(), timeout=5)  # Wait a bit
                ...
                self.error_logger.exception(f"⊗ inline-script unexpected error before process creation: {e}")
                return None
