...
def save_html_report(
    ...
    # HTML report is stored in the analytics/reports directory relative to logs_base_dir
    reports_dir = os.path.join(logs_base_dir, "analytics", "reports")
    os.makedirs(reports_dir, exist_ok=True)
    date_str = datetime.now().strftime("%Y-%m-%d")
    # The path from YAML: html_report_file has a format with {date}
    html_template = config.get("reports", {}).get("html_report_file", f"analytics/reports/analytics_{date_str}.html")
    report_file = os.path.join(logs_base_dir, html_template.replace("{date}", date_str)) if "{date}" in html_template else os.path.join(logs_base_dir, html_template)
